...
HRESULT APIENTRY MJPresent(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags) {
	...
	if (ShowMenu == true) {
		...
		ImGui::Begin("ELDEN MENU-By Killuano", &isOpen, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_MenuBar);
		...
		if (window == "window_home")
		{
			...
			ImGui::Checkbox("Dont Turn on-Kill", &showPlayerSpeed);
		...
		if (showAbout)
		{
			...
			if (showModPage)
			{
				ShellExecute(NULL, "open", "https://github.com/AmrHuss", NULL, NULL, SW_SHOWDEFAULT);
				showModPage = false;
			}
			style->Colors[ImGuiCol_Text] = ImColor(255, 255, 255);
			showTips = false;
